...
class CheckMachine:
  ...
  def auth(self, session: requests.Session, action: str, username: str, password: str):
    response = session.get(f"{self.url}/{action}")
    soup = self.c.get_soup(response, f"Invalid {action} page")
    self.c.assert_tags(
    ...
    self.c.get_soup(response, f"Failed to {action}")
    self.c.assert_in("session", session.cookies, f"No session after {action}")
    self.c.assert_eq(response.url.rstrip("/"), self.url, f"Invalid page after {action}")
  ...
  def ensure_orders(self, session: requests.Session, orders: list[Tuple[str, str]]):
    response = session.get(f"{self.url}/orders")
    soup = self.c.get_soup(response, "Invalid orders page")
    self.c.assert_tags(
